...
export const achievementDefs = [
  ...
  // Bestiary
  { id: 'bestiary_10',    name: 'Zoologist',         desc: 'Master 10 species',   req: () => Object.values(bestiaryState.entries).filter(e => e.kills >= 500).length >= 10, bonus: { all: 0.50 }, bonusDesc: '+50% All Stats' },
  { id: 'bestiary_30',    name: 'Apex Predator',     desc: 'Master 30 species',   req: () => Object.values(bestiaryState.entries).filter(e => e.kills >= 500).length >= 30, bonus: { all: 1.50 }, bonusDesc: '+150% All Stats' },
  ...
  // Extreme Scaling (Scientific Checks)
...
// Call every few seconds to check achievements
  ...
  achievementDefs.forEach(ach => {
    if (!achievementState.unlocked[ach.id]) {
      try {
        ...
        // Guard against broken req functions
      }
    }
  });
}
...
function applyBonus(bonus) {
  // drop bonus stored on character for use in reward system
